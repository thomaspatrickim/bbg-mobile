...
export default class TradesScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchTerm: '',
      ...
      apiResponse: '',
      locationSlug: '',
      tradesOpacity: new Animated.Value(0),
    };
    this._retrieveData()
  }
  ...
  loadTrades(locationSlug){
    //Get all trades for displaying
    axios({
      method: 'get',
      url: 'http://stage.bestbusiness.com/v1/locations/'+locationSlug+'/subcategories',
    })
    .then((res) => {
      this.setState({
      ...
      Animated.timing(this.state.tradesOpacity, {
        toValue: 1,
        duration: 750,
  ...
  performSearch(searchTerm) {
    axios({
      method: 'get',
      url: 'http://stage.bestbusiness.com/v1/locations/'+this.state.locationSlug+'/businesses/search/'+searchTerm,
    })
    .then((res) => {
      if(res.data[0].length < 1 && res.data[1].length < 1) {
        ...
        this.props.navigation.navigate('BusinessSearchResult', {businesses: res.data, title: searchTerm})
      }
    })
    .catch((error) => {
      console.log(error)
  ...
  render(props) {
    ...
    let header = <View style={styles.search}>
      ...
      <ImageBackground source={require('../../images/bg.png')} style={styles.search}>
      <Animated.View style={{flexDirection: 'column', alignItems: 'center', opacity: headerOpacity}}>
        ...
        <Text style={styles.logo_text_small}>Customer Recommended Businesses 2019</Text>
      ...
      <Animated.Text style={[styles.fadeInText, {opacity: titleOpacity}]}>All Trades A-Z</Animated.Text>
      </ImageBackground>
    </View>
  ...
  renderScroll(props) {
    ...
    return (
        <AnimatedSectionList
          style={{opacity: this.state.tradesOpacity}}
          ListHeaderComponent={<View style={styles.trades_header}><Text style={styles.trades_header_text}>ALL TRADES A-Z</Text></View>}
          renderItem={this._renderItem}
          renderSectionHeader={this._renderSectionHeader}
          sections={this.formatBusinesses()}
