...
class SearchResult extends Component {
  ...
  componentDidMount() {
    const businessesArray = this.props.navigation.getParam('businesses');
    let featured = false;
    let otherQualifiers = false;
    if(businessesArray[0].length >= 1) {
      featured = true;
    }
    if(businessesArray[1].length >= 1) {
      otherQualifiers = true;
    }
    this.setState({
      apiResponse: businessesArray[0],
      resultNumber: businessesArray[0].length + businessesArray[1].length,
      featured: featured,
      apiResponseOtherQualifiers: businessesArray[1],
